...
import { ValidatorFunction, IValidatorRule } from '@/types/forms';
...
export default class FormValidator {
	 ...
	 */
	createAlias(validatorName: string, aliasName: string) {
	 ...
	 */
	createAliases(aliases: Record<string, string>) {
		for(let validatorName in aliases) {
	...
	parseValidationRuleString(validationString: string): IValidatorRule[] {
		...
		return validationString.split('|').map(ruleString => {
			let parsed = ruleString.toLowerCase().split(':');
	...
	validate(validationString: string, value: string, label?: string): string | null {
		let rules = this.parseValidationRuleString(validationString);
		for(let rule of rules) {
			if(!this._validatorIsDefinedForRule(rule.validator)) continue;
			let err = this.validators[rule.validator]({value, param: rule.param, label});
	...
	private _validatorIsDefinedForRule(validatorName: string) {

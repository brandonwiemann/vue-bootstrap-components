...
export default class SearchResult {
	...
	_searchByKeyword(data) {
		...
		let props = this._getSearchableColumns(data);
		if(!props.length) return data;
		let q = this.query.keyword.toLowerCase();
		let results = [];
		for(let i=0;i<props.length;i++) {
			let prop = props[i];
			for(let j=0; j<data.length; j++) {
				let value = data[j][prop];
				if(!value) continue;
	...
	_paginate(data) {
		...
		let start = (this.query.pageNum - 1) * this.query.resultsPerPage;
